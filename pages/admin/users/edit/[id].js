...
const EditUser = () => {
  ...
  const {
    ...
    error,
    mutate,
  } = useSWR(router.query.id && `http://localhost:3000/api/admin/global?url=/users/${router.query.id}`);
  ...
  const onSubmit = async (data) => {
    try {
      setLoading(true);
      const res = await axios.put(`http://localhost:3000/api/admin/global?url=/users/${router.query.id}`, {
        ...data,
      });
